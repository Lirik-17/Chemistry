...
cbi = function (p, i_exp)
    ...
    png(plt_all, string("./case3/figs/i_exp_1", i_exp))
    return false
end
...
cb = function (p, loss_train, loss_val, g_norm)
    ...
    if iter % n_plot == 0
        ...
        # plot!(size=(400, 350))
        png(plt_loss, "./case3/figs/loss");

        plt_grad = plot(list_grad, xscale=:log10, yscale=:log10, label="grad_norm")
        png(plt_grad, "./case3//grad")

        @save "./case3/checkpoint/mymodel.bson" p opt list_loss_train list_loss_val list_grad iter
    end
    iter += 1
end

if is_restart
    @load "./case3/checkpoint/mymodel.bson" p opt list_loss_train list_loss_val list_grad iter
    iter += 1
end
